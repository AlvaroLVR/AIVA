...
export default function FinishBuying() {
  ...
  const generarOrder = (e) => {
    ...
    addDoc(orders, order) // setDoc(orders, obj, id)
      .then(resp => setNumeroCompra(resp._key.path.segments[1]))
        ...
        <div className='row justify-content-center'>
          {finishBuy ?
            ...
            <div className='col-md-4 d-flex flex-column align-self-center m-3 p-3 rounded-3 bg-white'>
              <p className='text-center fs-4'>Ya realizaste la compra! 
                <br/> 
                  <span className='text-uppercase'>{formData.name}</span>
                   <br/>  Gracias por elegirnos.
                <br />
                  Este es tu numero de compra: <span className='text-white bg-dark rounded-2 p-2'> {numeroCompra} </span>
              </p>
